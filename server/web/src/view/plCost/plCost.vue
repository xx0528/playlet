  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="消费金币" prop="costGold" width="120" />
        ...
        <el-table-column align="left" label="购买视频" prop="buyVideo" width="320" />
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updatePlCostFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form ref="elFormRef" :model="formData" label-position="right" :rules="rule" label-width="80px">
        ...
        <el-form-item label="消费金币:" prop="costGold">
          <el-input-number v-model="formData.costGold" style="width:100%" :precision="2" :clearable="true" />
        </el-form-item>
...
const getTableData = async() => {
  const table = await getPlCostList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ...
  const res = await deletePlCostByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updatePlCostFunc = async(row) => {
  const res = await findPlCost({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deletePlCostFunc = async(row) => {
  const res = await deletePlCost({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
