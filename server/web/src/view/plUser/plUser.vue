  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="最后登录时间" width="180">
        ...
        <el-table-column align="left" label="用户Id" prop="userId" width="180" />
        <!-- <el-table-column align="left" label="访客id" prop="guestId" width="120" /> -->
        <el-table-column align="left" label="累计充值" prop="recharge" width="120" />
        <el-table-column align="left" label="当前金币" prop="curGold" width="120" />
        <el-table-column align="left" label="解锁记录" prop="buyVideos" width="320" />
        <!-- <el-table-column align="left" label="注册时间" width="180">
          <template #default="scope">{{ formatDate(scope.row.registerTime) }}</template>
        </el-table-column> -->
        <el-table-column align="left" label="收藏剧集" prop="likeVideos" width="320" />
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updatePlUserFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form ref="elFormRef" :model="formData" label-position="right" :rules="rule" label-width="80px">
        ...
        <el-form-item label="当前金币:" prop="curGold">
          <el-input-number v-model="formData.curGold" style="width:100%" :precision="2" :clearable="true" />
        </el-form-item>
...
const getTableData = async() => {
  const table = await getPlUserList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ...
  const res = await deletePlUserByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updatePlUserFunc = async(row) => {
  const res = await findPlUser({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deletePlUserFunc = async(row) => {
  const res = await deletePlUser({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
